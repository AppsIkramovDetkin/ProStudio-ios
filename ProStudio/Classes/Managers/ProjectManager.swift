...
class ProjectManager {
	...
	func createUser(email: String, password: String, closure: @escaping VoidClosure) {
		Auth.auth().createUser(withEmail: email, password: password, completion: { (_, error) in
			let ref = Database.database().reference().child("users").child(email.formattedEmail())
			ref.child("dateAddedUnix").setValue(Date().timeIntervalSince1970)
